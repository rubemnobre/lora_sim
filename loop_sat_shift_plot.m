...
LDRO = false;
CENTER_FREQ = 915e6;
decider = @shift_observer2_decider;
B = 125000;
GB = 128000;
OSR = 2;
N_SYMBOLS = 50;
...
for j=1:N_RUNS
    ...
    for k=1:N_SYMBOLS
        ...
        ci_rw_log = abs(log10(ci(2)) - log10(ci(1)))/abs(log10(MIN_SER));
        % fprintf("errors = %d; ser = %f; ci w = %f; ci_rw_log = %f\n", errors(end), ser, ci(2) - ci(1), ci_rw_log);
        if ser > MIN_SER && not(ci_rw_log < CONV)
            not_good = true;
        end
        ci_ws(k) = ci_rw_log;
    end
...
savefig(gcf(), "loop_sat_shift_plot.fig");
exportgraphics(gcf(), "loop_sat_shift_plot.pdf", "ContentType", "vector");