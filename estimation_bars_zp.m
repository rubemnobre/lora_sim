...
for j=1:N_SHIFTS
    ...
    parfor i=1:N
        ...
        p = (mind-1)/ZP;
        if LDRO == false
            ests(i) = p - round(p, TieBreaker="tozero");
        else
            ests(i) = (p/4 - round(p/4, TieBreaker="tozero"));
        end
        if abs(ests(i)) == 0.5
            ests(i) = 0.5*randsample([-1 1], 1);
        end
    end
    means(j) = mean(ests);
    stdevs(j) = std(ests);
...
savefig(gcf(), "estimation_bars_zp.fig");
exportgraphics(gcf(), "estimation_bars_zp.pdf", "ContentType", "vector");